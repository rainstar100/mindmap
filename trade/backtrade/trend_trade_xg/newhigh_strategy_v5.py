        ...
        if self.monitor_startdate:
            ...
            if current_date<=self.monitor_enddate:
                ...
                if  self.data.high[0]/self.data.h30[-1]>=1.09:
                    print(f'{self.data.datetime.date(0)}-->Buy signal detected---?{self.buy_number}')
                    buy_price = self.data.close[0]*1.01
                    size = int(self.p.fixed_amount*self.buy_number/ buy_price/100)*100
    ...
    def notify_order(self, order):
        ...
        if order in self.order.values():
            ...
            if order.status in [order.Submitted]:
                ...
                if direction=='BUY':
                    self.buy_number=self.buy_number+1
