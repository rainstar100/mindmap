#buy 2 ,sell 1 when high point on 2nd day  instead of buy 1  43-->68 [601606]
#buy at 1% new point instead of at 9%  32-->43 [601606]
#sell_barket install sell
#连续提交2个订单，后一个会顶掉前一个
#第二天优先执行昨天条件的订单，再执行next代码
...
class NewHighStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.monitor_startdate is None:
            ...
            monitor_start_condition1=self.data.high[-1]==self.data.h30[-1] and self.data.volume[-1]==self.data.v90[-1]
            monitor_start_condition2=self.data.high[0]/self.data.h30[-1]>=1.09
            #check monitor start codition
            if monitor_start_condition1 and monitor_start_condition2:
                #set monitor start date and end date
                self.monitor_startdate=self.data.datetime.date(0)
        ...
        ####--->monitor sart is true, start to monitor
        #check monitor period
        #----->if monitor sart date is setted ,start to monitor
        if self.monitor_startdate :
            if current_date<=self.monitor_enddate:
    
                #try to buy at 9% above previous high
                print(f'{self.data.datetime.date(0)}-->place Buy  order for trying to buy during new high')
                buy_price = self.data.h30[-1]*1.01
                ...
                if self.position.size>0:
                    self.order['SELL']=self.sell(exectype=bt.Order.Limit,size=size/2,price=self.data.close[0]*1.09,valid=validity)
