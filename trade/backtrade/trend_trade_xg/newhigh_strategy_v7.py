...
class NewHighStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.position.size>0:
            sell_price=self.data.ma5[0]
            self.sell(exectype=bt.Order.Stop,size=self.position.size,price=sell_price,valid=validity)
        #place buy order
        ...
        if self.monitor_startdate is None:
            ...
            monitor_start_condition2=self.data.close[0]/self.data.h30[-1]>=1.09
        ...
        if self.monitor_startdate :
            if current_date<=self.monitor_enddate:
                ...
                buy_price = self.data.h30[-1]*1.09
                size = int(self.p.fixed_amount / buy_price/100)*100
                self.buy(exectype=bt.Order.Stop,size=size,price=buy_price,valid=validity)
